...
def NexonMaple(MapleName):
    ...
    # 메이플스토리 전적검색 서버 아이콘
    Find_Maple_charLink_Severlcon = Find_Maple_charLink.img['src']
    ...
    return {"HomeURL": NexonMapleHome_URL, "charLink": Find_Maple_charLink_Link,
            "ServerIcon": Find_Maple_charLink_Severlcon, "GuildName": Find_Maple_GuildName}
...
def MapleNormalData(MapleName):
    ...
    Read_NexonMapleData = Open_NormalMapleData.read()

    #BeautifulSoup 에서 html 형식으로 파싱
    Maple_Soup = BeautifulSoup(Read_NexonMapleData, "html.parser")
...
def MapleIndexSearch(MapleName):
    ...
    # 메이플 유니온 랭킹
    print("유니온 랭킹 : " + MapleRankingData_def["UnionRank"])
    # 업적랭킹
